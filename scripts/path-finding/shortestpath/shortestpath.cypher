...
WITH stops[index] AS location, stops[index-1] as previous, stops, index
RETURN location.current.id AS place, reduce(acc =0, distance in [stop in stops[0..index] | stop.distance] | acc + distance) AS cost
